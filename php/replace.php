    ...
    public static function commenti($commenti_array, $id_commenti_con_like, $isLogged)
    {
        ...
        if (!$isLogged) { //sostituisce l'input del "mi piace" con un link al login (un utente non loggato non pùò inserire "mi piace")
            ...
            foreach ($commenti_array as $commento) {
                ...
                if ($_SESSION['level'] == 'admin') //l'utente è un admin, aggiungo la possibilità di rimuovere il commento
                    $commenti.= "<label onclick=\"eliminaCommento('commento_$commento[id]')\" class=\"delete\" for=\"delete_$commento[id]\">Elimina commento</label><input id=\"delete_$commento[id]\" type=\"button\" value=\"delete\" name=\"elimina commento\" />";
                $commenti .= "<label class=\"miPiace upGray \" for=\"input_$commento[id]\" id=\"Label$commento[id]\" onclick=\"miPiace('Label$commento[id]')\" onmouseover=\"miPiaceOver('Label$commento[id]')\" onmouseout=\"miPiaceOut('Label$commento[id]')\">Mi piace</label><input  id=\"input_$commento[id]\" type=\"button\" value=\"up\" name=\"miPiace\" /><p class=\"numeroLike\">$commento[likes]</p></li>";
            }
        } else {
